...
const dataJson = require('../../fixtures/editPet.json')
                ...
                cy.request({
                    ...
                    cy.request({
                        ...
                        body: {
                            "id": petId,
                            "category": {
                                "id": dataJson.category.id,
                                "name": dataJson.category.name
                            },
                            "name": dataJson.name,
                            "photoUrls": dataJson.photoUrls,
                            "tags": [
                                {
                                    "id": dataJson.tags[0].id,
                                    "name": dataJson.tags[0].name
                                }
                            ],
                        ...
                        expect(res.status).to.eq(200)
                        expect(res.body.id).to.eq(petId)
                        expect(res.body.category).has.property('id', dataJson.category.id)
                        expect(res.body.category).has.property('name', dataJson.category.name)
